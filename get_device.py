...
class Optimai:
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if attempt < retries - 1:
                    ...
                    continue
                return self.print_message(access_token, proxy, Fore.RED,
                                          f"Registering Nodes Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}")
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if attempt < retries - 1:
                    ...
                    continue
                return self.print_message(access_token, proxy, Fore.RED,
                                          f"Update Uptime Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}")
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if attempt < retries - 1:
                    ...
                    continue
                return self.print_message(access_token, proxy, Fore.RED,
                                          f"Perform Chechk-In Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}")

    async def process_register_nodes(self, access_token: str, register_payload: str, uptime_payload: str,
                                     use_proxy: bool):
    ...
    async def process_accounts(self, account, use_proxy: bool):
        url = "https://api.optimai.network/devices?limit=10&sort_by=last_used_at"
        me_url = "https://api.optimai.network/auth/me?platforms=all"
        headers = {
            ...
            try:
                async with ClientSession(connector=connector, timeout=ClientTimeout(total=120)) as session:
                    async with session.get(url=url, headers=headers) as response:
                        ...
                        #     print(account, item['id'])
                        if data['items']:
                            device_id = data['items'][0]['id']
                        else:
                            device_id = None
                    async with session.get(url=me_url, headers=headers) as response:
                        response.raise_for_status()
                        data = await response.json()
                        user_id = data['user']['id']
                    print(user_id, device_id)
                    open('test.txt', 'a', encoding='utf-8').write(f"{user_id}|{device_id}\n")
                    return user_id, device_id
            except (Exception, ClientResponseError) as e:
                if attempt < retries - 1:
                    await asyncio.sleep(5)
                    continue
                return self.print_message(account, proxy, Fore.RED,
                                          f"Perform Chechk-In Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}")
        # tasks = []
        # tasks.append(asyncio.create_task(self.process_perform_checkin(access_token, use_proxy)))
        # tasks.append(
        #     asyncio.create_task(self.process_register_nodes(access_token, register_payload, uptime_payload, use_proxy)))
        # await asyncio.gather(*tasks)

    async def main(self):
        try:
            accounts = [ac.strip() for ac in open('tokens.txt', 'r', encoding='utf-8').readlines()]
            if not accounts:
